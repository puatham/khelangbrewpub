...
**แก้บั๊กแจ้งเตือนซ้ำ (19 ส.ค.)**: รันจริงครั้งแรก (execution 468) พบ Weizen ถูกแจ้งเตือนซ้ำ 2 ข้อความ — `Check Sensor Freshness` เป็น Postgres node ที่ n8n รัน query **1 ครั้งต่อ 1 input item** แต่ query ตัวนี้ self-contained (คืนทุก batch active เองอยู่แล้ว ไม่ได้ใช้ค่าจาก input) พอรับ 2 item (1 ต่อ batch จาก `Get Latest Readings`) จึงรัน 2 รอบได้ 4 แถวซ้ำกัน → `Build Sensor Alert` สร้าง 2 ข้อความเหมือนกัน แก้ด้วยการเปิด **Execute Once** (`executeOnce: true`) ที่ node นี้ — เป็น pattern ที่ต้องระวังทุกครั้งที่ต่อ Postgres/HTTP node ซึ่ง query/request ไม่ได้อิงค่าจาก input เข้ากับ node ที่ให้หลาย item

---

### 8.10 Pill ตายกลางคัน — เตือนแบตตอน start + ประมาณอุณหภูมิจากตู้ (task #62, 19 ส.ค.)

**ข้อจำกัดที่เป็นโจทย์**: Pill ปิดผนึกอยู่ในถังหมัก **เปลี่ยนแบตกลางคันไม่ได้** (เปิดถัง = เบียร์เจอออกซิเจน) ถ้าแบตหมดระหว่างหมักต้องอยู่กับมันจนจบ batch — เกิดขึ้นจริง 19 ส.ค. กับ Pill02 (Weizen) การ "เตือนให้เปลี่ยนแบต" ระหว่างหมักจึงเป็นคำเตือนที่ทำตามไม่ได้

**A) เช็คแบตตอน `/ferment_start` (จังหวะเดียวที่ทำอะไรได้จริง — ถังยังเปิดอยู่)**: `Create Batch` ดึง `battery_percent` ของ reading ล่าสุดของ Pill ตัวนั้นมาด้วย → `Build Followup Message` แสดงเสมอ และเตือนเป็นพิเศษถ้า **< 30%**: `⚠️ Pill01 แบตเหลือ 15% — ควรเปลี่ยนก่อนปิดถัง (ระหว่างหมักเปลี่ยนไม่ได้...)` ไม่บล็อกการสร้าง batch

**B) Pill ตาย → ประมาณอุณหภูมิเบียร์จากตู้ควบคุม**: `beer ≈ ตู้ + gap` โดย gap คือส่วนต่าง Pill-ตู้ ซึ่งจากข้อมูลจริงพบว่า**นิ่งมากเมื่อตู้คุมอุณหภูมิคงที่** (Weizen: target 19.5→gap 1.38±0.21, 19.0→1.84±0.42, 18.0→2.05±0.11, 17.0→1.34±0.25 = แม่นราว ±0.5°C) **แต่เชื่อไม่ได้เลยตอนกำลังไล่อุณหภูมิ** เพราะเบียร์มีมวลความร้อนตามตู้ไม่ทัน (ช่วง cold crash จริง gap แกว่ง +2.92 → −1.54 ใน 3.5 ชม.)

`Get Latest Readings`/`Get Simulated Readings` จึงเพิ่ม CTE `fallback_gap` ที่คัดเฉพาะจุดที่เชื่อได้ 2 เงื่อนไข: (1) **เบียร์นิ่งจริง** — เปลี่ยนจากจุดก่อนหน้า < 0.15°C (2) **ตู้ตั้ง target เดียวกับปัจจุบัน** (gap แปรตามระดับอุณหภูมิ ใช้ข้ามระดับไม่ได้) ย้อนหลังไม่เกิน 48 ชม.จากจุด Pill สุดท้าย ต้องได้ ≥3 จุดถึงจะใช้ พร้อมเพิ่ม `pill_stale_hours`, `pill_battery_percent` — ทดสอบ SQL จริงบน VPS: Hazy DIPA (Pill ปกติ) ได้ 19 จุด gap 1.69±0.08, Weizen (Pill ตาย) ได้ 1 จุด → **ประมาณไม่ได้ ซึ่งถูกต้อง** เพราะเซ็นเซอร์ตายตอนกำลังไล่ cold crash ยังไม่ทันนิ่ง (query 21-33ms)

`Build AI Prompt` แทรกคำเตือนเข้า prompt เมื่อ Pill ค้างเกิน 2 ชม. บอก AI ตรงๆ ว่าข้อมูลค้าง **ห้ามสรุปว่าเบียร์นิ่งแล้วจากการที่กราฟแบน (แบนเพราะไม่มีข้อมูลใหม่ ไม่ใช่เพราะการหมักหยุด)** พร้อมค่าประมาณถ้าคำนวณได้ หรือบอกว่าประมาณไม่ได้พร้อมสั่งให้ระบุความไม่แน่นอนใน reasoning — เป้าหมายคือให้ระบบ degrade อย่างซื่อสัตย์ แทนที่จะมั่นใจผิดๆ กับข้อมูลแช่แข็ง ข้อความ Discord แสดงเป็น `⚠️ Pill เงียบมา 9.0 ชม. (แบตหมด) — ค่าด้านล่างเป็นข้อมูลค้าง` + `gravity: 1.020 SG (ค้าง ไม่อัปเดตแล้ว)` + `อุณหภูมิเบียร์: ≈4.98°C (ประมาณจากตู้ควบคุม)` (หรือ "ประมาณไม่ได้") — gravity ไม่มีทางประมาณได้เลยเพราะตู้ควบคุมวัดไม่ได้ บอกตรงๆ ว่าเป็นค่าสุดท้ายก่อนเซ็นเซอร์ตาย

ทดสอบ Node harness ทั้ง pipeline 3 เคส (Pill ตาย+ประมาณได้ / Pill ตาย+ประมาณไม่ได้ / Pill ปกติ) ยืนยัน prompt กับข้อความ Discord ถูกต้องทุกเคส และเคส Pill ปกติไม่มีคำเตือนโผล่มารบกวน

---

### 8.16 "Temp Guard" — ตรวจอุณหภูมิทุก 15 นาทีด้วยโค้ดล้วน (task #69, 8 ก.ย.)

**ปัญหา**: เบียร์หลุดต่ำกว่าขีดล่างยีสต์ตั้งแต่ราว 09:00 แต่กว่าจะรู้คือรายงาน AI ตอน 18:01 — **ช้าไป 9 ชั่วโมง** เพราะการตรวจทุกอย่างผูกกับรอบ cron 4 ชม.

**แก้โดยเพิ่มชั้นตรวจที่ไม่ใช้ AI เลย** เกาะไปกับ `Telemetry Sync` ที่รันทุก 15 นาทีอยู่แล้ว — ขอบเขตพวกนี้คำนวณได้แน่นอน ไม่ต้องตีความ จึงไม่ควรเสียเวลารอ AI และไม่มีค่า token

```
Phase Analysis Cron  ทุก 4 ชม.    AI คิด     "อยู่เฟสไหน / ดีต่อเบียร์ไหม"
Temp Guard           ทุก 15 นาที  โค้ดล้วน   "เกินขอบเขตหรือยัง"
```

#### เกณฑ์มาจากการวัดข้อมูลจริง ไม่ได้เดา

วัดพฤติกรรมตู้จาก `temp_controller_readings` ทั้งหมดก่อนตั้งค่า:

| ตู้ | เบี่ยงจาก target หลัง target นิ่งเกิน 6 ชม. |
|---|---|
| Fridge | p50 0.10 · p95 1.63 · p99 2.75 · สูงสุด 7.08 |
| Fridge2 | p50 0.08 · p95 0.51 · p99 1.66 · สูงสุด 2.55 |

| Pill | อัตราเปลี่ยน °C/ชม. |
|---|---|
| Pill01 | p50 0.00 · p95 0.75 · p99 1.49 |

→ ตั้งเกณฑ์ตู้เบี่ยง **2.5°C** (เหนือ p99) และ Pill เปลี่ยน **1.5°C/ชม.** (≈p99)

**เจอเรื่องสำคัญระหว่างวัด**: `control_log` มีการเปลี่ยน target แค่ **5 ครั้งจาก 14 ครั้งจริง** — อีก 9 ครั้งผู้ใช้ปรับตรงในแอป RAPT ไม่ผ่านบอท ดังนั้นทุกเกณฑ์ที่ต้องรู้ว่า "target เพิ่งเปลี่ยนหรือยัง" **ต้องอ่านจาก `target_temperature_c` ใน `temp_controller_readings` เอง ห้ามอิง `control_log`**

#### 6 ย่านที่ตรวจ (เรียงตามลำดับความสำคัญ)

| kind | เงื่อนไข | ปุ่ม |
|---|---|---|
| `sensor_stale` | Pill หรือตู้เงียบเกิน 2 ชม. | ไม่มี |
| `controller_lag` | เบี่ยง >2.5°C ต่อเนื่อง 1 ชม. **และไม่ลดลง ≥0.3°C** | ไม่มี — บอกให้ไปดูของจริง |
| `rapid_change` | Pill ขยับ >1.5°C/ชม. ทั้งที่ target ไม่ได้เพิ่งเปลี่ยน | ไม่มี |
| `yeast_low` / `yeast_high` | Pill หลุดช่วงยีสต์เกิน 0.2°C | ✅ มี |
| `arrived` | Pill นิ่ง (<0.15°C/ชม.) หลัง target เปลี่ยนมา 2-12 ชม. | ไม่มี |

**เงื่อนไข "ไม่ลดลง" ของ `controller_lag` คือหัวใจ** — cold crash ใช้เวลาไล่ 8 ชม.+ (18/08 ตู้ลงจาก 19.48 → 7.06) ถ้าดูแค่ค่าเบี่ยงจะเตือนผิดทุกครั้งที่สั่ง ramp การเช็คว่าเบี่ยง**ลดลง**หรือไม่แยก "ตู้พัง" ออกจาก "กำลังทำตามคำสั่ง" ได้โดยไม่ต้องรู้เจตนา

**`arrived` คือสิ่งที่ระบบขาดมาตลอด** — เดิมคำนวณ ETA ว่า "น่าจะถึงประมาณ 20:00" แต่ไม่เคยยืนยันว่าถึงจริงหรือยัง นิยาม "ถึงแล้ว" ที่ใช้คือ **เบียร์หยุดขยับ** ซึ่งไม่ต้องรู้ `stable_gap_c` เลย เพราะการนิ่งคือนิยามของการถึงที่หมายอยู่แล้ว

#### ปุ่มปรับ target ทำงานได้แม้ไม่มี `stable_gap_c`

การแปลงแบบสัมบูรณ์ (`target = pill เป้าหมาย − gap`) ต้องมี gap ซึ่งมักเป็น `null` ในช่วง 2 ชม.แรกหลังปรับ target แต่ถ้าคิดเป็น**การขยับสัมพัทธ์** ไม่ต้องใช้ gap เลย:

```
target ใหม่ = target ปัจจุบัน + (pill เป้าหมาย − pill ปัจจุบัน)
```

ใช้สมมติฐานเดียวกัน (gap คงที่ตอนนิ่ง) แต่ gap หักล้างกันไปเอง จำกัดการขยับไม่เกิน ±2.0°C ต่อครั้ง และดันเข้ามาในช่วงยีสต์ 0.5°C ไม่ใช่จ่อขอบพอดี

**และเสนอได้เฉพาะตอนตู้ไล่ถึง target แล้วจริง** (`|ตู้ − target| ≤ 0.5°C`) — ถ้ายังไล่ไม่ถึง เบียร์กำลังจะขยับเองอยู่แล้ว การขยับ target ซ้ำตอนนั้นจะ over-correct (เจอจริง 8 ก.ย.: ตู้ 15.98 vs target 16.50 ถ้าไล่ถึงเบียร์จะขึ้นไป ~18.2 เองโดยไม่ต้องทำอะไร) เคสนี้จะขึ้นข้อความ `⏳ ยังไม่เสนอปรับ target` แทนปุ่ม

#### กันเตือนซ้ำ — ตาราง `temp_alert_state`

รันทุก 15 นาที ถ้าไม่มี state จะได้ 96 ข้อความ/วัน ตารางใหม่เก็บ 1 แถวต่อ batch (`alert_kind`, `alerted_at`, `alerted_value`) — เตือนซ้ำเฉพาะเมื่อ **เปลี่ยนชนิด / แย่ลง ≥0.5°C / ครบ cooldown 6 ชม.** และ**ลบแถวทิ้งเมื่อกลับเข้าเกณฑ์** เพื่อให้ครั้งหน้าที่หลุดเตือนได้ใหม่ (หลักการเดียวกับ `prep_alerted_for_phase`)

`sensor_stale` ส่ง `alerted_value` เป็น `NULL` โดยตั้งใจ ไม่งั้นอายุที่เพิ่มขึ้นเรื่อยๆ จะเข้าเงื่อนไข "แย่ลง ≥0.5" แล้วเตือนซ้ำทุกครึ่งชั่วโมง

การตัดสินใจ + บันทึก state ทำใน query เดียวด้วย data-modifying CTE (`INSERT ... ON CONFLICT DO UPDATE ... WHERE <ควรเตือนซ้ำ> RETURNING`) — ถ้า `WHERE` ไม่ผ่านจะไม่มีแถวคืนมา = ไม่ส่งอะไร ได้ dedupe แบบ atomic ไม่มี race

#### เลิกใช้ `Check Sensor Freshness` ใน cron

`sensor_stale` มาแทนสาย `Check Sensor Freshness` → `Build Sensor Alert` → `Send Sensor Alert` (ลบ 3 nodes, cron เหลือ 12) — ของเดิมเกณฑ์ 6 ชม. รันทุก 4 ชม. และ **ไม่มี dedupe เลย** (เตือนซ้ำทุกรอบตลอดไปตราบใดที่ sensor ยังเงียบ) ตัวใหม่รู้เร็วกว่า 16 เท่าและเงียบเมื่อควรเงียบ

⚠️ `Check Temp Bounds` ต้องเปิด **`executeOnce: true`** เพราะรับ item ต่อจาก INSERT ที่พ่นออกมาหลักร้อย — บั๊กแบบเดียวกับที่เคยทำให้ `Check Sensor Freshness` ยิง alert ซ้ำ 4 รอบ

#### `/ferment_band` — ช่วงอุณหภูมิที่ผู้หมักตั้งเอง (task #70)

ช่วงของยีสต์กว้างเกินจะมีประโยชน์ในทางปฏิบัติ — Verdant IPA รองรับ **18-25°C** แปลว่าเบียร์ไปอยู่ 24°C ก็ยัง "ผ่าน" ทั้งที่ผิดจากแผน (สูตรระบุ Primary 18°C) ไปไกลแล้ว จึงเพิ่มชั้นที่สอง:

```
⚠️ ขีดยีสต์   18-25°C      สเปกผู้ผลิต หลุดแล้วเสี่ยงยีสต์หยุด/fusel     (แรงกว่า)
👀 ช่วงที่ตั้ง 18.0-18.5°C   เจตนาของคนหมัก หลุดแล้วแค่ไม่ตรงแผน
```

`batches.beer_band_low_c` / `beer_band_high_c` (NULL = ไม่ตั้ง ใช้ช่วงยีสต์อย่างเดียวเหมือนเดิม) ตั้งผ่าน `/ferment_band pill:Pill01 low:18.0 high:18.5` ล้างด้วย `clear:true` — **เป็นอุณหภูมิของเบียร์ (Pill) ไม่ใช่ค่าที่ตั้งบนตู้** ซึ่งต่างกันเกือบ 2°C ระหว่างหมัก ระบบแปลงเป็น target ตู้ให้เองเวลาเสนอปรับ

- `band_low`/`band_high` อยู่ **ท้ายกว่า** `yeast_low`/`yeast_high` ในลำดับความสำคัญ ถ้าหลุดทั้งคู่จะแสดงอันที่แรงกว่า
- เผื่อขอบ 0.1°C (แคบกว่าของยีสต์ที่ 0.2 เพราะช่วงนี้แคบกว่ามาก) กันแกว่งไปมาตรงขอบ
- เสนอ target ที่เล็ง **กลางช่วง** ไม่ใช่จ่อขอบ จะได้ไม่หลุดออกอีกทันทีที่แกว่งนิดเดียว
- `Set Beer Band` เรียง low/high ให้เองถ้าใส่สลับมา และ **ล้าง `temp_alert_state` ของ batch นั้นทิ้ง** เพื่อให้เกณฑ์ใหม่ประเมินใหม่ตั้งแต่รอบถัดไป ไม่ติด dedupe ของเกณฑ์เดิม

**เลือกแบบตั้งค้างไว้ ไม่เลื่อนตามเฟสเอง** — สูตรมีอุณหภูมิคนละค่าในแต่ละช่วง (Primary 18 · D-rest 20→21→22 · Cold Crash 16→12→6→3) แต่ชื่อ step ในสูตรไม่ map ตรงกับ 7 เฟสของระบบ (`Hop Creep Rest (DH2)` คือเฟสอะไร?) การเดาจับคู่มีโอกาสผิด ส่วนผลข้างเคียงของการตั้งค้างคือ **ถ้าลืมตั้งใหม่ตอนเปลี่ยนเฟส ระบบจะเตือน** ซึ่งก็คือสิ่งที่ต้องการพอดี (เตือนว่ายังไม่ได้ยกอุณหภูมิเข้า d-rest)

**ทดสอบกับ DB จริง (rollback ทุกครั้ง)**: ใส่สลับ `low:18.5 high:18.0` → เก็บเป็น 18.0/18.5 ถูกต้อง · `clear:true` → กลับเป็น NULL · Pill ที่ไม่มี batch active → 0 แถว (ข้อความจับเคสนี้แล้ว) · ตั้ง 18.0-18.2 ตอนเบียร์ 18.31 → guard คืน `band_high` · ขยายเป็น 18.0-18.6 → คืน `arrived` · เคส `band_high` ห่างแค่ 0.11°C ไม่ขึ้นปุ่มเพราะต่ำกว่าเกณฑ์ 0.5°C แต่ยังแสดงข้อความอธิบายสถานการณ์

#### เตือนก่อนหลุด ไม่ใช่หลังหลุด + โหมดแจ้งทุกรอบ (task #71)

เดิมทุก kind เป็นแบบ **reactive** — รอให้ข้ามเส้นก่อนแล้วค่อยเตือน ผู้ใช้ชี้ว่าอยากได้ข้อสังเกตแบบที่มนุษย์ให้ เช่น *"ลงจาก 18.31 (21:45) มา 18.00 (22:40) ถ้าลงต่ออีกนิดจะได้ทั้ง band_low และเข้าใกล้ขีดล่างยีสต์"* ซึ่งเป็นการ**คาดการณ์** ไม่ใช่การรายงาน

**`drifting_low` / `drifting_high`** — หา slope ด้วย `regr_slope()` จาก 3 ชม.ล่าสุด (ไม่ใช่หัวท้าย 2 จุด เพราะ Pill อ่านเป็นขั้น 1/16°C จุดเดียวที่กระโดดจะลากเส้นเพี้ยนทั้งเส้น) แล้วคาดการณ์ว่าอีกกี่ชั่วโมงจะแตะขอบ เตือนเมื่อ **≤ 3 ชม.และยังไม่หลุด** ต้องเคลื่อนจริง (`|อัตรา| ≥ 0.08°C/ชม.`) และมีอย่างน้อย 4 จุด

ขอบที่ใช้คือ**ขอบที่จะชนก่อน** — `GREATEST(band_low, yeast_min)` ตอนไหลลง และ `LEAST(band_high, yeast_max)` ตอนไต่ขึ้น ข้อความบอกด้วยว่าขอบนั้นเป็นขีดของยีสต์หรือช่วงที่ตั้งเอง

**bug ที่เจอตอนทดสอบ**: เขียนเงื่อนไข `pill > floor_c` ตอนแรก ทำให้เคสที่เบียร์อยู่ **ตรงขอบพอดี** แล้วยังไหลลงต่อ (18.00 = ขีดล่างยีสต์ 18 พอดี อัตรา −0.121°C/ชม.) เงียบสนิท ทั้งที่เป็นเคสที่ต้องเตือนที่สุด — เปลี่ยนเป็น `GREATEST(..., 0)` แทน ส่วนเคสที่หลุดไปแล้วจริงๆ `band_low`/`yeast_low` อยู่ก่อนใน `CASE` จึงคว้าไปก่อนอยู่แล้ว

**ปรับ `MIN_NUDGE_C` 0.5 → 0.2°C**: เหตุผลเดิมที่ตั้ง 0.5 คือ "จมไปกับ hysteresis ของตู้" ซึ่งถูกแค่ครึ่งเดียว — hysteresis ทำให้ค่า *ณ ขณะหนึ่ง* แกว่ง แต่ไม่ได้ทำให้ *ค่าเฉลี่ยที่สมดุล* ขยับตาม การเลื่อน target 0.25°C จึงเลื่อนอุณหภูมิเบียร์ที่สมดุลไป 0.25°C จริง 0.2°C = 2 สเต็ปของความละเอียดที่ RAPT ตั้งได้ (เกณฑ์ 0.5 ของ Brew Master คงไว้ เพราะเสนอการแก้ระดับเฟสซึ่งเป็นการขยับหลักองศา)

**ส่วนที่เป็นดุลพินิจ ยกให้ Brew Master** — ประโยคอย่าง *"รอดูอีกสักรอบก่อนก็ได้ เพราะ high_krausen กำลังจะมาซึ่งจะดันอุณหภูมิขึ้นเอง"* โค้ดตัดสินไม่ได้ เพิ่มด้านที่ 5 "แนวโน้มและจังหวะ" ใน prompt พร้อมข้อมูล **2 หน้าต่างเวลา**:

```
· ช่วง 6 ชม. (ทิศทางรวม):      เพิ่มขึ้น 0.06°C/ชม.
· ช่วง 3 ชม. (ตอนนี้ไปทางไหน): ลดลง 0.12°C/ชม.
· ถ้าอัตราช่วง 3 ชม.เป็นแบบนี้ต่อไป จะแตะขอบ 18.00°C ในอีกราว 0.0 ชม.
ถ้าสองค่านี้ทิศทางต่างกัน แปลว่าเพิ่งผ่านจุดกลับตัว ให้ระบุไว้ใน notes ด้วย
```

ต้องมีทั้งคู่เพราะตอบคนละคำถาม — เคสจริง 8 ก.ย.: 6 ชม. **+0.06** แต่ 3 ชม. **−0.12** แปลว่าผ่านยอดมาแล้วกำลังไหลลง ซึ่งเห็นได้ก็ต่อเมื่อดูทั้งสองค่า ถ้าให้ค่าเดียวจะสรุปผิดทาง อัตราช่วง 3 ชม.ใช้หน้าต่างเดียวกับ Temp Guard จะได้ไม่ขัดกันเวลาอ่านคู่กัน (คำนวณด้วย least-squares ใน JS ให้ตรงกับ `regr_slope` ใน SQL)

**โหมดแจ้งทุกรอบ** — เก็บสวิตช์ใน `bot_state.temp_guard_report_mode` (`always` / `alerts`) **ไม่ hardcode** จะได้ปิดด้วย SQL บรรทัดเดียวโดยไม่ต้อง reimport:
```sql
UPDATE bot_state SET value='alerts' WHERE key='temp_guard_report_mode';
```
โหมด `always` ส่งแถว `ok` ทุก 15 นาที (96 ข้อความ/วัน) — แถวนี้ **ไม่แตะ `temp_alert_state` เลย** จึงไม่กวน dedupe/clear ของ alert จริง และไม่มีปุ่ม (บังคับ `wantPill = null`) ตาราง `bot_state` ที่ค้างมาจาก `Discord Command Intake` ที่ retire ไปแล้วได้กลับมามีประโยชน์พอดี

**แก้เพิ่มหลังเห็นข้อความจริงรอบแรก** — รอบ 23:15 ส่ง `drifting_low` ถูกต้อง (เบียร์ 18.06 ลดลง 0.12°C/ชม. จะแตะขอบล่าง 18.00 ในอีก ~0.5 ชม.) แต่ **ไม่มีปุ่ม** ทั้งที่ควรมี เจอ 2 bug:

1. **floating point** — ต้องขยับ +0.19 → ดันเป็นขั้นต่ำ 0.20 → target 16.50 แต่ `16.50 - 16.30` ใน JS ได้ `0.19999999999999929` ซึ่ง `< 0.2` เลยตกเกณฑ์ไปเฉยๆ ทั้งที่คำนวณมาถูกทุกขั้น — ปัดทศนิยม 2 ตำแหน่งก่อนเทียบ
2. **เสนอผิดทางได้** — สูตร `wantPill - pill` ให้ค่าติดลบเมื่อเบียร์ยังอยู่เหนือกลางช่วงแต่กำลังไหลลง (เช่น 18.40 ในช่วง 18.0-18.5) = สั่งลด target ซึ่ง**ยิ่งเร่งให้หลุด** — บังคับให้ทิศทางสวนกับการไหลเสมอ

และเปลี่ยนจาก "ตัดปุ่มทิ้งเมื่อการขยับเล็กกว่าขั้นต่ำ" เป็น **"ดันขึ้นเป็นขั้นต่ำ" เฉพาะเคส drift** — เพราะข้อความเตือนถูกส่งไปแล้วว่ากำลังจะหลุด การเตือนแล้วไม่บอกว่าให้ทำอะไรคือแย่ที่สุดของทั้งสองทาง ส่วนเคสที่หลุดแล้วแต่นิ่งยังใช้กติกาเดิม

ข้อมูลจริงยืนยันว่าควรมีปุ่ม: ส่วนต่าง Pill-ตู้นิ่งมากที่ **1.80°C** (6 จุดติด) ตู้คุมได้ที่ ~16.2 เทียบ target 16.30 → **จุดสมดุลของเบียร์อยู่ที่ ~18.05** ซึ่งคือขอบล่างพอดีและเป็นขีดล่างยีสต์ด้วย ไม่เหลือ margin เลย ขยับ target ขึ้น 0.2°C ย้ายจุดสมดุลไป ~18.25 กลางช่วงพอดี

**ทดสอบกับ DB จริง (rollback)**: โหมด `always` → ได้แถว `ok` · สลับเป็น `alerts` → 0 แถว · เบียร์ 18.00 ตรงขอบ อัตรา −0.121 → `drifting_low` พร้อมปุ่ม **16.55°C (+0.25)** ซึ่งตรงกับที่วิเคราะห์ด้วยมือไว้ก่อนหน้าเป๊ะ · harness เรนเดอร์ครบทั้ง `ok` / `drifting_low` (ทั้งเคสตรงขอบและเคสล่วงหน้า 2.6 ชม.) / `drifting_high`

**ทดสอบ**: สร้างตารางบน DB จริงแล้วรัน query 2 ครั้งติด — ครั้งแรกได้ `yeast_low` (Pill 17.69 ต่ำกว่า Verdant 18°C) ครั้งที่สองเงียบตาม dedupe · Node harness เรนเดอร์ครบทั้ง 6 kind + เคสตู้ยังไล่ไม่ถึง ยืนยันปุ่มขึ้นเฉพาะ `yeast_low`/`yeast_high` ตอนตู้นิ่งแล้ว และทุกข้อความอยู่ในลิมิต Discord
...
#### แก้หลังรันจริงรอบแรก (execution 667, 8 ก.ย. 18:01)

รันผ่านครบทั้ง 7 nodes ไม่มี parse error verdict `watch` และจับได้สิ่งที่ตัวแรกไม่เคยพูด — ที่คมที่สุดคือ *"กราฟขาดช่วงสูงสุด 3.8 ชม. ... อาจทำให้จุดชันสุดของ SG drop (ตัวบ่งชี้ high_krausen) ถูกมองข้ามไปแล้ว"* คือมันเชื่อมโยงเองว่าช่องว่างในข้อมูลอาจทำให้พลาดเฟสที่ผูกกับการเตือน dry hop พอดี แต่เจอ 4 จุดที่ต้องแก้:

1. **`Day` ใน dry hop ไม่ใช่กำหนดการ** — ผู้ใช้ยืนยันว่า Day 3 คือ "ประมาณช่วง high_krausen" และ Day 9 คือ "รอบ dry hop ที่สอง" เท่านั้น แต่ prompt เดิมเขียนว่า *"Day นับจากวัน pitch — ดูจำนวนวันที่หมักมาแล้วด้านล่างเทียบ"* จึงตอบออกมาว่า "อีก ~29 ชม.ถึง dry hop Day 3" ซึ่งไม่มีความหมาย และถ้าหมักช้ากว่าแผนเพราะอุณหภูมิต่ำ (กำลังเป็นอยู่จริง) การเตือนตามวันที่จะสั่งใส่ hop ผิดจังหวะ — เปลี่ยน render เป็น `รอบที่ N (สูตรเขียนไว้ราว Day X)` และสั่งชัดว่า **ตัวสั่งคือเฟสจริงจากกราฟ รอบแรก = `high_krausen` รอบท้าย = หลัง `fg_stable`** แก้ทั้ง prompt ตัวแรกและ brewmaster
2. **`phase_doubt` ถูกใช้ผิดวัตถุประสงค์** — prompt สั่งว่าถ้าไม่สงสัยให้ตอบ `null` แต่มันเขียนย่อหน้า *ยืนยันว่าไม่สงสัย* มาแทน แล้วโค้ดเอาไปแสดงใต้หัวข้อ "❓ สงสัยเรื่องเฟส" จนอ่านแล้วขัดกันเอง — แยกเป็น `phase_agrees` (boolean) + `phase_doubt_reason` แล้ว **บังคับในโค้ด**ว่าโชว์เหตุผลได้เฉพาะตอน `phase_agrees === false` เท่านั้น (defense-in-depth แบบเดียวกับ guard `next_phase !== phase`)
3. **มันคำนวณเลขผิดและขัดแย้งกับตัวเอง** — เขียนว่า *"Beer temp 17.69°C ... แต่ยังอยู่ในช่วงยีสต์ Verdant รองรับ (18-25°C)"* ทั้งที่ 17.69 ต่ำกว่าขีดล่าง 0.31°C บทเรียนเดียวกับ ETA: **อะไรที่คิดเป็นตัวเลขได้ ให้โค้ดคิด** เพิ่ม `yeast_range_status` ที่คำนวณใน `Build AI Prompt` แล้วยื่นข้อสรุปให้เลย (`ต่ำกว่าขีดล่างของยีสต์ 0.31°C (เบียร์ 17.69°C, ยีสต์รองรับ 18-25°C)`) พร้อมกำกับว่า "คำนวณให้แล้ว ห้ามคำนวณเอง"
4. **ยก hysteresis ปกติขึ้นเป็น risk** — เตือนว่า "ตู้ควบคุมต่ำกว่า target ถึง ~0.27°C" ทั้งที่ตู้คุมแบบตัด-ต่อ ค่าต่างระดับนี้เป็นปกติ — บอก prompt ว่า ±0.5°C คือปกติ เตือนเฉพาะตอนห่างผิดปกติหรือไล่ไม่ถึงเป็นเวลานาน
...
### 8.20 `/ferment_event` — บันทึกสิ่งที่ทำจริง ไม่ใช่แผนในสูตร (10 ก.ย.)

กฎ hop creep ที่เพิ่มไว้ในข้อ 8.17 เขียนว่า *"ถ้าเพิ่ง dry hop ไปไม่นาน ให้เฝ้า gravity ต่อ"*
แต่ระบบ**ไม่มีทางรู้ว่า "เพิ่ง" หรือเปล่า** — รู้แค่ `dry_hop_plan` จากสูตรซึ่งเป็นตัวเลข Day
ที่คนเขียนสูตรประมาณไว้ และเราตกลงกันแล้วว่าห้ามใช้นับวันตัดสินอะไร กฎจึงแทบไม่มีผล

```
/ferment_event pill:Pill01 type:🌿 Dry hop date:9/9/2026 18:00 note:รอบ 1 Citra+Amarillo
```

`type` เป็นเมนูให้เลือก (`choices` ของ Discord) กันพิมพ์ผิดจนได้ชนิดที่ระบบไม่รู้จัก
มี `↩️ ยกเลิกรายการล่าสุด` ไว้แก้ตอนกดผิด และตอบกลับพร้อมไทม์ไลน์ทั้ง batch ทุกครั้ง
จะได้เห็นทันทีว่าบันทึกผิดวันไหม ไม่ต้องมีคำสั่งดูรายการแยก

**ตัวอ่านวันที่แยกจากของเดิมโดยตั้งใจ** — `parseStartDate` ที่คำสั่งอื่นใช้ ถ้าอ่านไม่ออก
จะเงียบๆ ใช้เวลาปัจจุบันแทน ซึ่งยอมรับได้ตอนเริ่ม batch แต่อันตรายกับการบันทึกย้อนหลัง
(dry hop เมื่อวาน 18:00 กลายเป็นเวลาที่กดคำสั่ง = ผิดไป 20 ชม.ในข้อมูลที่ใช้ตัดสิน hop creep)
`parseEventDate` จึงฟ้องกลับแทน

ตรวจครบใน SQL ชั้นเดียว: หา batch ที่ active, กันวันอนาคต, กันวันก่อนเริ่มหมัก, undo —
ทดสอบทั้ง 6 เส้นทางบน DB จริงแบบ rollback แล้ว

⚠️ **CTE ทั้งหมดในคำสั่งเดียวเห็น snapshot ก่อนการลบ** ไทม์ไลน์ที่คืนมาหลัง undo จึงยังมี
รายการที่เพิ่ง DELETE อยู่ — ตัดออกฝั่ง JS ไม่งั้นผู้ใช้กด undo แล้วเห็นของเดิม นึกว่าลบไม่ติด

**ที่ข้อมูลนี้ไปโผล่**
- `batch_snapshot()` คืนคอลัมน์ `batch_events` → prompt ฝั่งข้อมูล (ไม่กระทบ cache prefix)
- เตือน hop creep เฉพาะเมื่อ dry hop ล่าสุดอยู่ใน 72 ชม. (BA Hop Creep Technical Brief)
- Grafana annotation เส้นแนวตั้งบนกราฟ — เห็นด้วยตาว่า gravity เปลี่ยนพฤติกรรมหลัง dry hop ไหม

⚠️ `batch_events` มี FK ชี้ `batches` ต้องอยู่ใน `TABLES` ของ `clear-data.sh` ไม่งั้น TRUNCATE พัง
(บทเรียนเดียวกับ `temp_alert_state`) — ทดสอบ TRUNCATE จริงแบบ rollback แล้ว

**เตือน hop creep ตอนลงมือ ไม่ใช่ตอนครบเวลา** — Temp Guard มี alert kind `hop_creep_watch`
ที่ยิงเมื่อ target ถูกลดลงไป ≤ 10°C (กำลังเข้า cold crash) ขณะที่ dry hop ล่าสุดยังไม่พ้น 72 ชม.
เลือกเงื่อนไขนี้เพราะการเตือนตามนาฬิกาจะกลายเป็นเสียงรบกวน ส่วนจังหวะที่ความเสียหายเกิดจริง
คือตอนตัดสินใจ crash/บรรจุ ไม่ใช่ตอนเวลาผ่านไปเฉยๆ — คำเตือนแบบเบากว่ามีอยู่แล้วใน prompt
ของรอบวิเคราะห์ 4 ชม. ตัวนี้ไม่มีปุ่มปรับอุณหภูมิ เพราะปัญหาไม่ได้อยู่ที่ค่าที่ตั้งแต่อยู่ที่จังหวะเวลา

**กันบันทึกซ้ำ**: กดคำสั่งซ้ำหลังเจอ error เป็นเรื่องธรรมชาติที่สุด และตอนทดสอบก็เกิดจริง —
ได้ `dry_hop` เวลาเดียวกัน 2 แถว ทำให้ prompt นับเป็น "รอบที่ 1" กับ "รอบที่ 2" ทั้งที่ใส่ฮอป
รอบเดียว แล้ว AI จะไม่เตือนรอบที่ยังไม่ได้ทำ ตอนนี้ถ้าชนิดและเวลาตรงกับที่มีอยู่แล้วจะตอบว่า
"บันทึกไว้แล้ว" แทนการเพิ่มซ้ำ

### 8.21 ยุบ IF 7 ตัวเป็น Switch (10 ก.ย.)

สายใน Discord webhook เคยเป็น IF ต่อกันเป็นแถว ทุกคำสั่งใหม่ = IF เพิ่มอีกตัวท้ายแถว
7 ตัวที่ route ตาม `$json.action` ยุบเป็น `Route Command` (Switch) ตัวเดียว 7 output
เหลือ 57 node จาก 63 ส่วน IF อีก 9 ตัวที่เหลือไม่ได้ route ตาม action (ตรวจลายเซ็น, ping,
component, autocomplete, หา batch เจอไหม) จึงคงไว้

**ยืนยันปลายทางเดิมทีละเส้นก่อนรื้อ** ไม่ใช่เชื่อว่าจำถูก — สคริปต์เทียบ `connections` ของ IF
ทั้ง 7 ตัวกับปลายทางที่ตั้งใจ ถ้าไม่ตรงจะหยุดทันที

⚠️ **ความเสี่ยงของการยุบคือ route ผิดช่องแล้วเงียบ** ไม่มี error ผู้ใช้เห็นแค่ "is thinking..."
ค้าง จึงเพิ่มกฎที่ 8 ใน validator: ทุกค่าที่ `actionMap` สร้างได้ต้องมี rule รองรับ และ rule
ที่ `actionMap` ไม่เคยสร้างก็ถือว่าผิด (สะกดผิดหรือค้างจากคำสั่งที่ถอดไปแล้ว) — ทดสอบทั้งสองทาง
...
- #69 Temp Guard — ตรวจอุณหภูมิเบียร์ทุก 15 นาทีด้วยโค้ดล้วน (ไม่ใช้ AI ไม่มีค่า token) ต่อท้าย `Telemetry Sync` 6 ย่าน: เซ็นเซอร์เงียบ / ตู้คุมไม่อยู่ / เปลี่ยนเร็วผิดปกติ / หลุดช่วงยีสต์ (มีปุ่มปรับ target) / ถึงเป้าแล้ว — เกณฑ์ทุกตัววัดจากข้อมูลจริงของตู้ (p99) ไม่ได้เดา พร้อมตาราง `temp_alert_state` กันเตือนซ้ำ และเลิกใช้ `Check Sensor Freshness` ใน cron (ช้ากว่า 16 เท่าและไม่มี dedupe) — ✅ เสร็จ ทดสอบ query จริงบน VPS + harness ครบ 6 kind รอ **import + publish** `Telemetry Sync` + `Phase Analysis Cron` และรัน DDL `temp_alert_state` — 8 ก.ย.
- #70 `/ferment_band` ตั้งช่วงอุณหภูมิของเบียร์ที่ยอมรับได้ต่อ batch (`batches.beer_band_low_c`/`beer_band_high_c`) — ช่วงยีสต์ 18-25°C กว้างเกินจะจับความผิดปกติจริง เพิ่มชั้นที่แคบกว่าและสะท้อนเจตนาของเฟสนั้น เตือนแบบ 👀 เฝ้าดู (เบากว่า ⚠️ หลุดสเปกยีสต์) พร้อมปุ่มที่เล็งกลางช่วง ตั้งค้างไว้ไม่เลื่อนตามเฟสเอง — ✅ เสร็จ ทดสอบ SQL จริงบน VPS (rollback) + harness ทุกข้อความ รอ **import + publish** `Telemetry Sync` + `Discord Interactions Webhook` และรัน `Register Slash Commands` ใหม่ — 8 ก.ย.
- #71 เตือน**ก่อน**หลุด (`drifting_low`/`drifting_high` คาดการณ์จาก slope 3 ชม. เตือนเมื่อจะแตะขอบใน ≤3 ชม.) + Brew Master เห็นแนวโน้ม 2 หน้าต่างเวลา (6 ชม./3 ชม.) จึงให้ดุลพินิจเรื่องจังหวะได้ + โหมดแจ้งทุกรอบผ่านสวิตช์ `bot_state.temp_guard_report_mode` (เปิด `always` ไว้ช่วงแรก ปิดด้วย SQL บรรทัดเดียว) — ✅ เสร็จ ทดสอบ SQL จริงบน VPS + harness ทุก kind รอ **import + publish** `Telemetry Sync` + `Phase Analysis Cron` + `Phase Analysis Engine` + `Discord Interactions Webhook` — 8 ก.ย.
